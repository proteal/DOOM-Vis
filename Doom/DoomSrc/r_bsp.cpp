...
void AddSegInfo(seg_t* line, vector<SegInfo>& segInfo, int parentIdx)
{
	segInfo.emplace_back();
	SegInfo& info = segInfo.back();

	info.seg = *line;
	info.parent = parentIdx;
}
...
void R_Subsector(int num, vector<SegInfo>& segInfo, int parentIdx)
{
	...
	while (count--)
	{
		R_AddLine(line, segInfo, parentIdx);
		line++;
	}
}
