...
qboolean         precache = true;        // if true, load all graphics at start 
...
qboolean*	mousebuttons = &mousearray[1];		// allow [-1]
...
void G_DoSaveGame(void)
{
	...
	*save_p++ = static_cast<byte>(gamemap);
	for (i = 0; i < MAXPLAYERS; i++)
		*save_p++ = static_cast<byte>(playeringame[i]);
...
void G_DoPlayDemo(void)
{
	...
	gameaction = ga_nothing;
	demobuffer = demo_p = reinterpret_cast<byte*>(W_CacheLumpName(defdemoname, PU_STATIC));
	if (*demo_p++ != VERSION)
	{
		doomCallbacks->Printf("Demo is from a different game version!\n");
		gameaction = ga_nothing;
		return;
	}
