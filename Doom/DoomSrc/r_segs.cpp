...
void R_RenderSegLoop(void)
{
	...
	for (; rw_x < rw_stopx; rw_x++)
	{
		...
		if (markceiling)
		{
			...
			if (top <= bottom)
			{
				ceilingplane->top[rw_x] = static_cast<byte>(top);
				ceilingplane->bottom[rw_x] = static_cast<byte>(bottom);
			}
		}
		...
		if (markfloor)
		{
			...
			if (top <= bottom)
			{
				floorplane->top[rw_x] = static_cast<byte>(top);
				floorplane->bottom[rw_x] = static_cast<byte>(bottom);
			}
		}
...
int	stop)
{
	...
	if (segtextured)
	{
		...
		if (offsetangle > ANG180)
			offsetangle = -static_cast<int>(offsetangle);
