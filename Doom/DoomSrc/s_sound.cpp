...
void S_Init(int sfxVolume, int musicVolume)
{
	...
	doomCallbacks->InitSound(numChannels);
	...
	channels = reinterpret_cast<channel_t*>(Z_Malloc(numChannels*sizeof(channel_t), PU_STATIC, 0));
...
void S_StartSound(void* origin_p, int sfx_id)
{
	...
	mobj_t*	origin = reinterpret_cast<mobj_t*>(origin_p);
...
{
	return handle >= 0 && channels[handle].sfxinfo != nullptr;
}
	...
	for (cnum = 0; cnum < numChannels; cnum++)
	{
		...
		if (c->sfxinfo)
		{
			if (I_SoundIsPlaying(c->handle))
			{
				...
				{
					audible = S_AdjustSoundParams(listener, reinterpret_cast<mobj_t*>(c->origin), &volume, &sep);
					...
					{
						doomCallbacks->UpdateSound(c->handle, volume, sep);
					}
				}
			}
			else
...
void S_SetMusicVolume(int volume)
{
	...
	musicVolume = volume;
	doomCallbacks->SetMusicVolume(volume);
}
...
void S_ChangeMusic(int musicnum, int looping)
{
	...
	// play it
	doomCallbacks->PlaySong(reinterpret_cast<const ubyte*>(music->data), W_LumpLength(music->lumpnum), looping != 0);
