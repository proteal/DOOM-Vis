...
fixed_t	y)
{
	...
	if (x >= 0)
	{
		...
		else
		{
			...
			{
				// octant 8
				return -static_cast<int>(tantoangle[SlopeDiv(y, x)]);
			}
			else
			{
				// octant 7
...
void R_RenderPlayerView(player_t* player)
{
	...
	vector<BspNodeInfo> nodeInfo;
	vector<SegInfo> segInfo;
	// The head node is the last node output.
	R_RenderBSPNode(numnodes - 1, nodeInfo, segInfo, -1, true);
	doomCallbacks->UpdateRenderData(nodeInfo, segInfo);
