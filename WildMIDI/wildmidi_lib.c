...
static int WM_LoadConfig(const char *config_file) {
	...
	if ((config_buffer = (char *) WM_BufferFile(config_file, &config_size))
			== NULL) {
		WM_FreePatches();
		return -1;
	}
	...
	if (dir_end != NULL) {
		...
		if (config_dir == NULL) {
			WM_ERROR(__FUNCTION__, __LINE__, WM_ERR_MEM, "to parse config",
					errno);
		...
		if (config_buffer[config_ptr] == '\r') {
			...
			if (config_ptr != line_start_ptr) {
				if ((line_tokens = WM_LC_Tokenize_Line(
						&config_buffer[line_start_ptr])) != NULL) {
					if (strcasecmp(line_tokens[0], "dir") == 0) {
						if (config_dir) {
							free(config_dir);
						}
						config_dir = _strdup(line_tokens[1]);
						if (config_dir == NULL) {
							WM_ERROR(__FUNCTION__, __LINE__, WM_ERR_MEM,
									"to parse config", errno);
		...
		if (!(nte->modes & SAMPLE_ENVELOPE)) {
			...
			if (nte->env < 5) {
				...
				if (nte->env_level > nte->sample->env_target[5]) {
					nte->env_inc = -(signed long)(nte->sample->env_rate[5]);
				} else {
			...
			if (nte->env < 3) {
				...
				if (nte->env_level > nte->sample->env_target[3]) {
					nte->env_inc = -(signed long)nte->sample->env_rate[3];
				} else {
			...
			if (nte->env_level > nte->sample->env_target[4]) {
				nte->env_inc = -(signed long)nte->sample->env_rate[4];
			} else {
...
static inline unsigned long int get_inc(struct _mdi *mdi, struct _note *nte) {
	...
	return (((freq / ((WM_SampleRate * 100) / 1024)) * 1024
			/ nte->sample->inc_div));
}
...
static void do_note_on(struct _mdi *mdi, struct _event_data *data) {
	...
	if (nte->active) {
		...
		nte->env = 6;
		nte->env_inc = -(signed long)nte->sample->env_rate[6];
		nte = nte->replay;
	} else {
		if (mdi->note_table[1][ch][note].active) {
			...
			mdi->note_table[1][ch][note].env_inc =
				-(signed long)mdi->note_table[1][ch][note].sample->env_rate[6];
		} else {
			nte_array = mdi->note;
			if (nte_array == NULL) {
				mdi->note = nte;
							...
							if (note_data->modes & SAMPLE_CLAMPED) {
								if (note_data->env < 5) {
									...
									if (note_data->env_level
										...
										note_data->env_inc =
											-(signed long)note_data->sample->env_rate[5];
									} else {
										note_data->env_inc =
								...
								if (note_data->env_level
									...
									note_data->env_inc =
										-(signed long)note_data->sample->env_rate[4];
								} else {
									note_data->env_inc =
				...
				case 0xF: // Meta Event
					if (current_event == 0xFF) {
						...
						if (sysex_store[sysex_store_len - 1] == 0xF7) {
							unsigned char tmpsysexdata[] = { 0x41, 0x10, 0x42,
									0x12 };
			...
			if (__builtin_expect((note_data != NULL ), 1)) {
				while (note_data != NULL) {
					...
					if (note_data->env_level
							> note_data->sample->env_target[note_data->env]) {
						note_data->env_inc =
							-(signed long)note_data->sample->env_rate[note_data->env];
					} else {
						note_data->env_inc =
...
int WildMidi_Close(midi * handle) {
	...
	if (first_handle == NULL) {
		WM_ERROR(__FUNCTION__, __LINE__, WM_ERR_INVALID_ARG, "(no midi's open)",
				0);
		return -1;
	}
	WM_Lock(&mdi->lock);
